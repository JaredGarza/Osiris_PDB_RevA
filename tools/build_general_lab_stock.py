"""General-purpose lab inventory proposal, separate from any frozen board BOM."""
...
sections=[]
def add(title, intro, headers, rows, notes): sections.append((title,intro,headers,rows,notes))
add('1 / Shared lab stocking strategy',
...
('Package standard','Proposed default for NEW general-purpose designs: 0603 EIA passives. Keep an 0402 satellite stock for compact Osiris work once the machine is confirmed capable. Keep 0805/1206 where power, capacitance or voltage requires them. This policy does not change any existing PCB footprint.'),
('Ryan\'s books','Inventory the books by value, package, rating, quantity, MPN and carrier. Keep assorted values for tuning and hand rework. Count them as machine stock only if identifiable and feedable. A book does not by itself replace continuous tape/reels.'),
('What makes this save time','Use a shared approved-parts library with stock IDs, footprints, datasheets and known carrier settings. Design new boards around these parts when electrically appropriate; reuse validated circuit blocks. Avoid a different regulator, connector and package on every project.'),
('Quantity interpretation','Targets are PER value, color, voltage option or selected IC variant unless stated otherwise. Subtract verified usable stock. Allow separate feeder setup/repair stock. Prices, supplier stock, factory reel quantities and packaging fees need quotes; no spend total is claimed.')])

add('2 / Passives: the reel inventory',
...
],[('Avoid unnecessary breadth','Do not buy full reels of every E-series value. Ryan\'s books can cover experiments; machine-ready stock should favor repeat values. The targets above establish useful depth without claiming every item is already shared across project BOMs.'),('Reel quotation','Ask for target quantity on a custom reel AND factory reel total price, including reeling fees, minimum packs and available continuous length. Final manufacturer part numbers and carrier dimensions must appear on the order.')])

add('3 / Common ICs: a reusable shelf',
...
],[('Design once, reuse','For each stocked IC: store symbol, footprint, pin mapping, decoupling requirements and a tested example circuit. Order by complete manufacturer code, not just a familiar name; gain, voltage, pinout and packaging variants matter.'),('Upgrade to larger reels','When repeated builds establish demand, increase the selected IC stock target. Keep expensive/specialized devices at small quantities until recurring consumption justifies more.')])

add('4 / Discretes, connectors and tools',
...
],[('Do not buy short-lived supplies in bulk','Buy solder paste to the near-term assembly schedule and manufacturer storage/shelf-life requirements. Stencils are specific to released PCB/panel layouts.'),('Machine-ready does not mean machine-only','Many headers and power connectors need hand or through-hole assembly. Reserve a separate work step and tools; verify actual machine capabilities for any nonstandard carrier.')])

add('5 / Purchasing and replenishment',
...
],[('Starter reorder rule','Until usage data exists, review weekly and flag core passives at 25% of target or below; flag core ICs at 10 units or below. Reorder to target after subtracting open purchase orders. These are provisional rules, not measured consumption forecasts.'),
('Once consumption is known','Reorder point = expected use during supplier lead time + safety stock. Available stock = on-hand usable stock minus project reservations. Include setup losses in consumption and increase safety stock for long or variable lead times.'),
('Tomorrow\'s boss request','Approve a shared P1 stock budget separate from project BOM purchases, assign the inventory/machine owners, and authorize distributor quotes. P2 expands after lab platform choices. The existing 27-code PDB list remains a separate procurement attachment.'),
('Sources / checked September 14, 2026','Manufacturer families: https://www.ti.com/product/TLV755P ; /TLV9002 ; /TLV3201 ; /SN74LVC1G14 ; /SN74LVC1G125 ; /SN74LVC1T45 ; /TPS22918 ; /INA180 ; /TCAN332 (all on www.ti.com/product/). Packaging: https://www.digikey.com/en/resources/product-services and https://forum.digikey.com/t/details-about-digi-reels/1303 . Stock quantities and priorities are planning recommendations, not manufacturer guidance.')])

styles=getSampleStyleSheet()
styles.add(ParagraphStyle(name='Small',fontName='Helvetica',fontSize=8.5,leading=11,spaceAfter=7))
styles.add(ParagraphStyle(name='Cell',fontName='Helvetica',fontSize=8,leading=10))
styles['Title'].fontSize=20
styles['Title'].textColor=colors.HexColor('#163c50')
styles['Heading2'].fontSize=11
styles['Heading2'].spaceBefore=9
styles['Heading2'].spaceAfter=4
story=[]; md=['# General lab component procurement plan','']
def p(t,style='Small'): return Paragraph(escape(t),styles[style])
for i,(title,intro,headers,rows,notes) in enumerate(sections):
    if i: story.append(PageBreak())
    story += [p(title,'Title'),p(intro)]
    data=[[p(x,'Cell') for x in headers]]+[[p(x,'Cell') for x in row] for row in rows]
    table=Table(data,colWidths=[112,278,126],repeatRows=1)
    table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#dae9ed')),('ROWBACKGROUNDS',(0,1),(-1,-1),[colors.white,colors.HexColor('#f1f5f6')]),('VALIGN',(0,0),(-1,-1),'TOP'),('TOPPADDING',(0,0),(-1,-1),6),('BOTTOMPADDING',(0,0),(-1,-1),6)]))
    story.append(table)
    md += ['## '+title,'',intro,'','| '+' | '.join(headers)+' |','|---|---|---|']
    md += ['| '+' | '.join(row)+' |' for row in rows]
    for head,body in notes:
        story += [p(head,'Heading2'),p(body)]
        md += ['', '### '+head,'',body]
    md += ['']
out=ROOT/'output/pdf/General-Lab-Component-Procurement-Plan.pdf'
(ROOT/'docs/GENERAL-LAB-COMPONENT-PROCUREMENT.md').write_text('\n'.join(md),encoding='utf-8')
def footer(c,d):
    c.setFont('Helvetica',8);c.drawString(48,24,'Shared lab stock | Proposal, not purchase order | 14 September 2026');c.drawRightString(564,24,str(d.page))
SimpleDocTemplate(str(out),pagesize=(612,792),leftMargin=48,rightMargin=48,topMargin=32,bottomMargin=40).build(story,onFirstPage=footer,onLaterPages=footer)
r=PdfReader(out)
print(f'{len(r.pages)} pages: {out}')
assert len(r.pages)==5, 'Review overflow before delivery'
